...
def main():
    ...
    rows = [
        ...
        ["Достоинства и недостатки", len(by_type.get("feature", []))
         - len(originals.PLAYERS_GUIDE_MERITS), "—", "канон не сверялся"],
        ["Достоинства из Руководства", len(originals.PLAYERS_GUIDE_MERITS),
         len(originals.PLAYERS_GUIDE_MERITS), "канон сверён по вики"],
    ]
    ...
    parts.append(
        f"## Что дальше{NL}{NL}"
        f"1. **Фоны из Руководства** — раздел на стр. 111–119. Основная "
        f"часть повторяет книгу правил, нового немного: «Страницы истории» "
        f"с недостатком «Линия крови» и «Торговля долгами» к Мавле. Сложность "
        f"в том, что фоны в компендиуме собраны в одну запись на все пять "
        f"точек, а книга их разносит: это дополнение существующих записей, "
        f"а не новые.{NL}"
        f"2. **Достоинства каитиффов, слабокровных и гулей** — глава "
        f"«Кастомы», стр. 126, 134 и 141. Часть уже перенесена как "
        f"достоинства и недостатки слабокровных.{NL}{NL}"
