...
Проверяется четыре вещи, и каждая заведена по конкретному случаю:
...
def at_hand(table):
    ...
    """
    return sum(1 for book, names in table.items() if book in AT_HAND
               for name in names if name not in originals.NOT_IN_RU)
