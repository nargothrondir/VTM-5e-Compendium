...
FOLDER_NAMES = {
    ...
    "Активи": "Активы",
    "Архаїчний": "Анахронизм",
    "Вади Тонкокровних": "Недостатки слабокровных",
    "Вживання речовин": "Употребление психоактивных веществ",
    "Вплив": "Влияние",
    "Зовнішність": "Внешность",
    "Контакти": "Информаторы",
    "Криваві узи": "Узы крови",
    "Лінгвістика": "Полиглот",
    "Мавла (або Противник)": "Мавла (или конкурент)",
    "Маска": "Маска",
    "Міфічні Переваги та Вади": "Фольклорные достоинства и недостатки",
    "Особливості": "Особенности",
    "Переваги Тонкокровних": "Достоинства слабокровных",
    "Переваги та Вади Притулку": "Достоинства и недостатки убежища",
    "Потребує Маски (••)": "Требует Маски (••)",
    "Притулок": "Убежище",
    "Ресурси": "Богатство",
    "Слава": "Слава",
    "Слуги": "Подручные",
    ...
    "Стадо": "Стадо",
    "Статус": "Статус",
    "Харчування": "Пищевые привычки",
}

# Уже переведённые названия должны проходить повторную сборку без ошибок.
FOLDER_NAMES.update({ru: ru for ru in list(FOLDER_NAMES.values())})
...
PACKS = {
    ...
    "advantages-flaws": (
        mapping_merits.ALL, {"feature"}, {"merit_entry", "merit_category"},
    ),
}
...
NOT_ENTRIES = {
    ...
    # Вводные абзацы категорий, чьи записи перенесены поштучно: сводной
    # записи в компендиуме им не отвечает, отдельным текстом они не нужны.
    "АНАХРОНИЗМ",
    "ВНЕШНОСТЬ",
    "ПИЩЕВЫЕ ПРИВЫЧКИ",
    "УЗЫ",
    ...
    "Достоинства и недостатки",   # обрывки двухстрочного титула
    "слабокровных",               # «ДОСТОИНСТВА СЛАБОКРОВНЫХ»
    "Страницы истории",           # перекрёстная ссылка, а не запись
}
...
def main():
    ...
    for pack, (table, entry_types, section_kinds) in PACKS.items():
        ...
        for name in by_name.keys() - used:
            ...
            # Ступени фонов входят в сводную запись своей категории, а не
            # переносятся поодиночке, — «не использован» про них не говорим.
            if section.get("category") in used:
                continue
            problems.append(f"{pack}: раздел книги не использован: "
                            f"{name!r} (с.{section['page']})")
