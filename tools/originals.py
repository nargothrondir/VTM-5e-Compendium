...
# Правка машинного перевода названий. Руководство переведено фанатами, и
# часть названий не просто неудачна, а сбивает с толку: «Проверить ствол» —
# это Check the Trunk, багажник машины с инструментом, а вовсе не оружие.
#
# Правится только название; текст записи остаётся как в книге, и источник
# в ней помечен. Ключ — как стоит в книге, значение — как читается в записи.
# Там, где книжное название просто неудачно, но не врёт, оно оставлено:
# «Ангел» вместо Ingénue, «Потускневшая аура» вместо Unseemly Aura.
RENAMES = {
    # Достоинства и недостатки, стр. 120–123
    "Режим Ньюит": "Режим Нюи",                    # Nuit Mode, фр. «ночь»
    "Проверить ствол": "Проверь багажник",         # Check the Trunk
    "Сторонний хастлер": "Подработка",             # Side Hustler
    "Темперированная Воля": "Закалённая Воля",     # Tempered Will
    "Транспарент": "Прозрачный",                   # Transparent
    "Венозная лента": "Вскрытая вена",             # Vein Tapper
    "Устаревающий упадок": "Голодное разложение",  # Starving Decay
    "Долги по кредитам": "Долги по благам",        # Prestation Debts
    # Глава «Кастомы»
    "Перемешник": "Пересмешник",                   # Mockingbird
    "Солнцезащитный": "Солнцестойкий",             # дневной ходок
    "Животное витэ": "Оскверняющая витэ",          # Befouling Vitae
    "Долговая яма": "Долговой раб",                # Debt Peon
    "Ходячий омен": "Ходячее знамение",            # Walking Omen
    "Слова-карриды": "Меченный словами",           # Word-Scarred
    "Выраканный на солнце": "Выцветший на солнце",  # Sun-Faded
    "Сверхъестественное присутствие": "Сверхъестественная примета",
    "Сумерочное присутствие": "Сумеречное присутствие",
    "Надежность": "Неуязвимость к Вере",           # Faith-Proof
    "Осознанный мечтатель": "Осознанный сновидец",  # Lucid Dreamer
    "Быстрый питатель": "Быстрый едок",            # Swift Feeder
    # Котерии
    "Отверстия для болтов": "Норы",                # Bolt Holes
}

# Таблицы оригиналов ключуются тем названием, которое видно в записи, —
# правка применяется к ним же, иначе глоссарий ссылался бы на имена,
# которых в компендиуме уже нет.
PLAYERS_GUIDE_MERITS = {RENAMES.get(k, k): v
                        for k, v in PLAYERS_GUIDE_MERITS.items()}
