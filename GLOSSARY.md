# Глоссарий

Терминология русского компендиума: **термин — оригинал — контекст**. Собрана
по ходу перевода и **сгенерирована из тех же данных, по которым собран
модуль**, — расходиться с содержимым паков она не может. Перегенерируется
командой `npm run glossary`.

Зачем: перевод шёл не пословно, а сопоставлением с официальным русским
изданием, поэтому названия сплошь и рядом далеки от оригинала. Глоссарий
фиксирует, какое решение принято и почему.

Здесь только переведённое. Что ещё предстоит и что для этого нужно —
в [плане перевода](ROADMAP.md).

Английские названия сверены по спискам
[paradoxwikis](https://vtm.paradoxwikis.com/Clans), а не взяты по памяти:
проверка находила расхождения — у бруха проклятие называется Violent Temper,
а не Rage; у Министерства — Abhors the Light, а не Light Sensitive.

## Как выбиралась терминология

1. **Приоритет за официальным изданием.** Если книга даёт слово, берётся её
   слово, даже когда оно далеко от оригинала: Unerring Aim — «Безупречная
   точность», Baal's Caress — «Ласка Ваала».
2. **Где официального перевода нет — слово ищется в словаре самой книги,**
   а не в общем. Так появились названия проклятий.
3. **Ничего не выдумано молча.** Каждое решение, где источник не дал прямого
   ответа, перечислено в разделе «Спорные решения».


---

## Структурные термины

Подписи, которыми размечены блоки внутри записей.

| Термин | Оригинал | Где встречается |
|---|---|---|
| Амальгама | Amalgam | силы, требующие второй Дисциплины |
| Расплата | Cost | силы |
| Пул | Dice Pool | силы |
| Правила | System | силы |
| Длительность | Duration | силы |
| Компоненты | Ingredients | ритуалы-обереги |
| Изъян | Bane | кланы |
| Одержимость | Compulsion | кланы (в компендиум пока не внесена) |
| Сила Крови | Blood Potency | отдельный пак |
| Тип охотника | Predator Type | отдельный пак |
| Голод | Hunger | сквозной термин |
| Человечность | Humanity | сквозной термин |
| Становление | Embrace | сквозной термин |
| Сородичи | Kindred | сквозной термин |
| витэ | vitae | сквозной термин |

Слово «Кровь» в значении игрового термина книга пишет с заглавной
(«испытание Крови», «Сила Крови»), в бытовом значении — со строчной.


---

## Дисциплины

| Термин | Оригинал | Код в системе |
|---|---|---|
| Алхимия слабокровных | Thin-blood Alchemy | `alchemy` |
| Анимализм | Animalism | `animalism` |
| Величие | Presence | `presence` |
| Доминирование | Dominate | `dominate` |
| Кровавое чародейство | Blood Sorcery | `sorcery` |
| Метаморфозы | Protean | `protean` |
| Мощь | Potence | `potence` |
| Ритуалы | Blood Sorcery Rituals | `rituals` |
| Сокрытие | Obfuscate | `obfuscate` |
| Стойкость | Fortitude | `fortitude` |
| Стремительность | Celerity | `celerity` |
| Ясновидение | Auspex | `auspex` |

---

## Силы Дисциплин, ритуалы и формулы

Переведено 121. Страница — по Книге правил.

### Алхимия слабокровных

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Длинные руки | Far Reach | 1 | 286 |
| Марево | Haze | 1 | 287 |
| Нечестивая иерогамия | Profane Hieros Gamos | 1 | 288 |
| Смог | Envelop | 2 | 287 |
| Восстановление крови | Defractionate | 3 | 288 |
| Импульс | Airborne Momentum | 4 | 289 |
| Будильник | Awaken the Sleeper | 5 | 289 |

### Анимализм

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Фамулус | Bond Famulus | 1 | 247 |
| Чутьё на зверя | Sense the Beast | 1 | 247 |
| Язык животных | Feral Whispers | 2 | 248 |
| Зверский аппетит | Animal Succulence | 3 | 248 |
| Трупный улей | Unliving Hive | 3 | 248 |
| Усмирение зверя | Quell the Beast | 3 | 249 |
| Поглощение духа | Subsume the Spirit | 4 | 249 |
| Отчуждение зверя | Drawing Out the Beast | 5 | 249 |
| Царь зверей | Animal Dominion | 5 | 249 |

### Величие

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Благоговение | Awe | 1 | 251 |
| Угроза | Daunt | 1 | 251 |
| Незабываемый поцелуй | Lingering Kiss | 2 | 251 |
| Очарование | Entrancement | 3 | 251 |
| Устрашающий взор | Dread Gaze | 3 | 252 |
| Призыв | Summon | 4 | 252 |
| Притягательный голос | Irresistible Voice | 4 | 252 |
| Звезда эфира | Star Magnetism | 5 | 252 |
| Преклонение | Majesty | 5 | 253 |

### Доминирование

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Принуждение | Compel | 1 | 254 |
| Провал в памяти | Cloud Memory | 1 | 254 |
| Внушение | Mesmerize | 2 | 255 |
| Помешательство | Dementation | 2 | 255 |
| Внедрённая директива | Submerged Directive | 3 | 255 |
| Забвение | Forgetful Mind | 3 | 255 |
| Оправдание | Rationalize | 4 | 255 |
| Приказ на самоуничтожение | Terminal Decree | 5 | 256 |
| Стадный инстинкт | Mass Manipulation | 5 | 256 |

### Кровавое чародейство

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Вечерняя бодрость | Wake with Evening's Freshness | 1 | 278 |
| Вкус крови | A Taste for Blood | 1 | 274 |
| Едкая кровь | Corrosive Vitae | 1 | 275 |
| Защитный круг от гулей | Warding Circle against Ghouls | 2 | 279 |
| Истончение витэ | Extinguish Vitae | 2 | 275 |
| Касание скорпиона | Scorpion's Touch | 3 | 275 |
| Сгущение крови | Blood of Potency | 3 | 276 |
| Хищение крови | Theft of Vitae | 4 | 276 |
| Котёл крови | Cauldron of Blood | 5 | 276 |
| Ласка Ваала | Baal's Caress | 5 | 276 |

### Метаморфозы

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Глаза зверя | Eyes of the Beast | 1 | 257 |
| Лёгкость пёрышка | Weight of the Feather | 1 | 257 |
| Оружие зверя | Feral Weapons | 2 | 257 |
| Слияние с землёй | Earth Meld | 3 | 257 |
| Смена облика | Shapechange | 3 | 258 |
| Перевоплощение | Metamorphosis | 4 | 258 |
| Превращение в туман | Mist Form | 5 | 258 |
| Свободное сердце | The Unfettered Heart | 5 | 258 |

### Мощь

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Мощный прыжок | Soaring Leap | 1 | 259 |
| Смертоносность | Lethal Body | 1 | 259 |
| Сокрушение | Prowess | 2 | 259 |
| Грубое насыщение | Brutal Feed | 3 | 260 |
| Искра ярости | Spark of Rage | 3 | 260 |
| Мёртвая хватка | Uncanny Grip | 3 | 260 |
| Глоток могущества | Draught of Might | 4 | 260 |
| Землетрясение | Earth Shock | 5 | 261 |
| Кулак Каина | Fist of Caine | 5 | 261 |

### Ритуалы

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Защитная печать от гулей | Ward Against Ghouls | 1 | 278 |
| Паучьи лапы | Clinging of the Insect | 1 | 278 |
| Постижение крови | Blood Walk | 1 | 278 |
| Путеводный камень | Craft Bloodstone | 1 | 278 |
| Защитная печать от духов | Ward against Spirits | 2 | 279 |
| Истина в крови | Truth of Blood | 2 | 279 |
| Очи вавилонские | Eyes of Babel | 2 | 279 |
| Свечение следопыта | Illuminate Trail of Prey | 2 | 279 |
| Связь с сиром | Communicate with Kindred Sire | 2 | 280 |
| Защитная печать от люпенов | Ward against Lupines | 3 | 280 |
| Защитный круг от духов | Warding Circle against Spirits | 3 | 280 |
| Зов Дагона | Dagon's Call | 3 | 280 |
| Отторжение губительного древа | Deflection of Wooden Doom | 3 | 280 |
| Пожиратель пламени | Firewalker | 3 | 280 |
| Эссенция воздуха | Essence of Air | 3 | 281 |
| Бесплотный путь | Incorporeal Passage | 4 | 281 |
| Защита неприкосновенного убежища | Defense of the Sacred Haven | 4 | 281 |
| Защитная печать от каинитов | Ward against Cainites | 4 | 282 |
| Защитный круг от люпенов | Warding Circle against Lupines | 4 | 282 |
| Око ночного ястреба | Eyes of the Nighthawk | 4 | 282 |
| Врата истинного святилища | Escape to True Sanctuary | 5 | 282 |
| Жало неизбежной погибели | Shaft of Belated Dissolution | 5 | 283 |
| Защитный круг от каинитов | Warding Circle against Cainites | 5 | 283 |
| Каменное сердце | Heart of Stone | 5 | 283 |

### Сокрытие

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Безмолвие смерти | Silence of Death | 1 | 262 |
| Плащ теней | Cloak of Shadows | 1 | 262 |
| Незримая поступь | Unseen Passage | 2 | 262 |
| Маска тысячи лиц | Mask of a Thousand Faces | 3 | 263 |
| Электронный призрак | Ghost in the Machine | 3 | 263 |
| Бесследное исчезновение | Vanish | 4 | 263 |
| Маскировка | Conceal | 4 | 263 |
| Личина самозванца | Impostor's Guise | 5 | 264 |
| Тайное собрание | Cloak the Gathering | 5 | 264 |

### Стойкость

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Сила жизни | Resilience | 1 | 265 |
| Твёрдость духа | Unswayable Mind | 1 | 265 |
| Живучие звери | Enduring Beasts | 2 | 265 |
| Непрошибаемость | Toughness | 2 | 265 |
| Превозмогание проклятия | Defy Bane | 3 | 266 |
| Укрепление сознания | Fortify the Inner Façade | 3 | 266 |
| Глоток упорства | Draught of Endurance | 4 | 266 |
| Горнило боли | Prowess from Pain | 5 | 266 |
| Мраморная плоть | Flesh of Marble | 5 | 267 |

### Стремительность

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Быстрая реакция | Rapid Reflexes | 1 | 267 |
| Кошачья грация | Cat's Grace | 1 | 268 |
| Проворство | Fleetness | 2 | 268 |
| Быстрее ветра | Blink | 3 | 268 |
| Траверс | Traversal | 3 | 269 |
| Безупречная точность | Unerring Aim | 4 | 269 |
| Глоток изящества | Draught of Elegance | 4 | 269 |
| Молниеносный удар | Lightning Strike | 5 | 269 |
| Скорость мысли | Split Second | 5 | 269 |

### Ясновидение

| Термин | Оригинал | Ур. | Стр. |
|---|---|---|---|
| Обострение чувств | Heightened Senses | 1 | 270 |
| Потустороннее зрение | Sense the Unseen | 1 | 271 |
| Предвестие | Premonition | 2 | 271 |
| Познание души | Scry the Soul | 3 | 271 |
| Слияние чувств | Share the Senses | 3 | 272 |
| Психометрия | Spirit's Touch | 4 | 272 |
| Вселение | Possession | 5 | 272 |
| Дальновидение | Clairvoyance | 5 | 273 |
| Телепатия | Telepathy | 5 | 273 |

---

## Кланы и проклятия

| Клан | Изъян | Оригинал | Альтернативный изъян | Оригинал |
|---|---|---|---|---|
| Бану Хаким | Кровавая зависимость | Blood Addiction | Ядовитая кровь | Noxious Blood |
| Бруха | Буйный нрав | Violent Temper | Насилие | Violence |
| Вентру | Изысканный вкус | Rarefied Tastes | Иерархия | Hierarchy |
| Гангрел | Звериные черты | Bestial Features | Инстинкты Выживания | Survival Instincts |
| Геката | Болезненный Поцелуй | Painful Kiss | Распад | Decay |
| Каитиф | Изгой | Outcast | — | — |
| Ласомбра | Искажённое отражение | Distorted Image | Черствость | Callousness |
| Малкавиан | Расколотое восприятие | Fractured Perspective | Неестественные проявления | Unnatural Manifestations |
| Министерство | Неприятие света | Abhors the Light | Хладнокровный | Cold-Blooded |
| Носферату | Отвратительность | Repulsiveness | Заражение | Infestation |
| Равнос | Обречённость | Doomed | Нерожденное Имя | Unbirth Name |
| Салюбри | Гонимые | Hunted | Аскетизм | Asceticism |
| Слабокровные | — | — | — | — |
| Тореадор | Жажда красоты | Aesthetic Fixation | Мучительное сочувствие | Agonizing Empathy |
| Тремер | Ущербная Кровь | Deficient Blood | Украденная кровь | Stolen Blood |
| Цимисхи | Привязанность | Grounded | Проклятая учтивость | Cursed Courtesy |

---

## Одержимости

В записи компендиума пока не внесены. Семь названий — из Книги правил (стр. 212—213), семь — из Руководства для игроков.

| Клан | Термин | Оригинал |
|---|---|---|
| Бану Хаким | приговор | Judgment |
| Бруха | бунт | Rebellion |
| Вентру | превосходство | Arrogance |
| Гангрел | животные порывы | Feral Impulses |
| Геката | болезненность | Morbidity |
| Ласомбра | безжалостность | Ruthlessness |
| Малкавиан | наваждение | Delusion |
| Министерство | преступление | Transgression |
| Носферату | криптомания | Cryptophilia |
| Равнос | искушение судьбы | Tempting Fate |
| Салюбри | аффективное сопереживание | Affective Empathy |
| Тореадор | восхищение | Obsession |
| Тремер | перфекционизм | Perfectionism |
| Цимисхи | алчность | Covetousness |

---

## Типы охотника

| Термин | Оригинал |
|---|---|
| Бестия | Blood Leech |
| Вымогатель | Extortionist |
| Джентльмен | Consensualist |
| Идол | Osiris |
| Искуситель | Siren |
| Лазутчик | Trapdoor |
| Монтеро | Montero |
| Морфей | Sandman |
| Мрачный жнец | Grim Reaper |
| Налётчик | Alleycat |
| Преследователь | Pursuer |
| Расхититель могил | Graverobber |
| Семьянин | Cleaver |
| Суррогатчик | Bagger |
| Тусовщик | Scene Queen |
| Фермер | Farmer |

---

## Сила Крови

| Термин | Оригинал |
|---|---|
| Сила Крови 0 (ноль): слабая кровь | Blood Potency 0 |
| Сила Крови 1 | Blood Potency 1 |
| Сила Крови 2 | Blood Potency 2 |
| Сила Крови 3 | Blood Potency 3 |
| Сила Крови 4 | Blood Potency 4 |
| Сила Крови 5 | Blood Potency 5 |
| Сила Крови 6 и выше | Blood Potency 6 |

---

## Достоинства и недостатки

Всего 85 записей. Английских названий здесь нет: компактного сверенного списка для них найти не удалось, а восстанавливать по памяти — ровно тот риск, которого проект избегал.

| Термин | Рейтинг | Стр. |
|---|---|---|
| Алхимик | — | 185 |
| Без убежища | • | 195 |
| Беззубый | — | 185 |
| Библиотека | • | 196 |
| Богатство | — | 186 |
| Влияние | — | 186 |
| Восхитительный | •••• | 181 |
| Высокофункциональный наркоман | • | 184 |
| Дневной хищник | — | 185 |
| Дурная слава | — | 190 |
| Жажда мафусаила | • | 182 |
| Живучесть вампира | — | 185 |
| Жуткое место | • | 195 |
| Зависимость | •• | 184 |
| Зависимость от витэ | — | 185 |
| Известное местоположение | •• | 195 |
| Информаторы | — | 187 |
| Ищейка | • | 182 |
| Камера для заключённых | • | 196 |
| Клановый изъян | — | 185 |
| Клеймо Камарильи | — | 185 |
| Конкурент | — | 189 |
| Красивый | •• | 181 |
| Крепкий желудок | ••• | 182 |
| Лаборатория | • | 196 |
| Мавла | — | 187 |
| Магическая защита | • | 196 |
| Маска | — | 189 |
| Мишень для кольев | •• | 184 |
| Мёртвая плоть | — | 185 |
| Надуманная опасность | • | 184 |
| Надуманное препятствие | • | 184 |
| Невосприимчивость к узам | •••• | 184 |
| Неграмотный | •• | 183 |
| Неистовый Зверь | — | 185 |
| Непопулярный | • | 187 |
| Неустойчивые узы | •• | 183 |
| Нищий | • | 186 |
| Омерзительный | •• | 181 |
| Операционная | • | 196 |
| Отверженный | •• | 194 |
| Отверженный среди анархов | — | 185 |
| Охрана | • | 196 |
| Повадки хищника | •• | 192 |
| Подозрительный | • | 194 |
| Подручные | — | 189 |
| Пожиратель органов | •• | 181 |
| Покойник | • | 189 |
| Покровитель в Камарилье | — | 185 |
| Полиглот | — | 182 |
| Полный жизни | — | 185 |
| Потенциал Крови | — | 185 |
| Презираемый | •• | 187 |
| Преследователи | • | 190 |
| Пристрастие | • | 184 |
| Пристрастие к узам | • | 183 |
| Продвинутая система безопасности | • | 196 |
| Пустотелый | •• | 189 |
| Пути отступления | • | 196 |
| Раб кровавых уз | •• | 183 |
| Разборчивость | • | 182 |
| Ретроград | • | 181 |
| Роскошь | • | 196 |
| Сверхъестественная проблема | • | 195 |
| Склонность к Дисциплине | — | 186 |
| Слава | — | 190 |
| Союзники (надёжность) | — | 191 |
| Союзники (эффективность) | — | 191 |
| Способность есть пищу | •• | 184 |
| Стадо | — | 192 |
| Статус | — | 192 |
| Стигматы | • | 184 |
| Страшная тайна | — | 190 |
| Стёртая личность | • | 189 |
| Субтильность смертного | — | 185 |
| Тайник с оружием | • | 196 |
| Товарищи среди анархов | — | 186 |
| Убежище | — | 194 |
| Уродливый | • | 181 |
| Устойчивость к узам | • | 183 |
| Устойчивые узы | • | 183 |
| Фальсификатор | • | 189 |
| Фермер | •• | 182 |
| Ходячий анахронизм | •• | 181 |
| Эксклюзивное расположение | • | 196 |

---

## Приведение названий из Руководства

Руководство для игроков зовёт Дисциплины по-своему; в компендиуме приведено к основной книге.

| Как в основной книге | Как в Руководстве |
|---|---|
| Кровавое чародейство | КОЛДОВСТВО КРОВИ |
| Мощь | МОГУЩЕСТВО |
| Обливион | ОБЛИВИОН |

---

## Имена собственные

Заголовки в книге набраны капсом, и при переводе в обычный регистр эти слова сохраняют заглавную.

| Слово |
|---|
| Ваала |
| Дагона |
| Каина |

---

## Спорные решения

Случаи, где источник не дал прямого ответа и решение принято осознанно.

**Названия клановых проклятий.** В оригинале имя есть у каждого проклятия,
но ни основная книга, ни Малая книга знаний их не приводят — ставят заголовок
«Изъян» и сразу текст. Похоже на упрощение при переводе. Названия переведены
с канонических английских, причём в словаре самой книги: четыре она подарила
дословно — «болезненный Поцелуй», «отвратительные», «жаждут красоты»,
«привязанность». Остальные собраны из её же лексики: «Буйный нрав» при «едва
удаётся сдерживать свою ярость», «Обречённость» при «Равнос обречены»,
«Гонимые» при «на салюбри охотятся».

**«Обливион».** Дисциплины Oblivion в основной книге нет вовсе, а «Забвение»
там занято силой Доминирования (Forgetful Mind). Взято слово из Руководства
для игроков — единственного источника, который её описывает. Придумывать
термин показалось хуже.

**«Изъян» против «Одержимости».** Руководство подписывает «Клановый изъян: X»
то, что на деле является Одержимостью: текст под этим заголовком описывает
принуждение на одну сцену. Официальное название термина — «Одержимость»
(Книга правил, стр. 210). В компендиуме сохранено официальное разделение.

**«Изъян» на обоих заголовках вкладки.** Основное проклятие книга
озаглавливает «Изъян», и так же названа вкладка в самом Foundry, а вариант
из Руководства первое время подписывался «Альтернативное проклятие». Два
абзаца одной вкладки называли одно и то же разными словами. Оставлен «Изъян»:
он совпадает и с книгой, и с интерфейсом.

**Имя клана — с заглавной.** Издание пишет его то так, то этак: «на салюбри
охотятся» и «Салюбри получают штраф» стоят в одной записи. Это имя
собственное, и регистр выровнен по нему. Курсивом имя не отмечается — в
описании клана оно встречается на каждом шагу.

**Клановые достоинства котерии.** В сводных перечнях вики их нет: они лежат
отдельным разделом «Coterie Clan Merits», до которого запрос по достоинствам
котерии не доходит. Пятнадцать названий сверены по нему; шестнадцатое,
Mortal Heart для слабокровных, в русском Руководстве отсутствует.

**«Торговля долгами» осталась без оригинала.** Раздел Руководства об учёте
благ и долгов; Prestation Debts занято другой записью — это недостаток
«Долги по благам», — а своего имени у раздела в каноне не нашлось. Пустая
строка здесь честнее правдоподобной догадки.

**Машинный перевод названий выправлен.** Руководство переведено фанатами,
и часть названий не просто неудачна, а сбивает с толку: «Проверить ствол» —
это Check the Trunk, багажник машины с инструментом, а не оружие;
«Перемешник» — Mockingbird, то есть пересмешник; «Слова-карриды» —
Word-Scarred, где «scarred» принято за имя. Двадцать одно название
приведено к смыслу оригинала (`RENAMES`), текст записи при этом оставлен
как в книге и помечен источником. Там, где книжное название просто
неудачно, но не врёт, оно оставлено: «Ангел» вместо Ingénue.

**«Мощь», а не «Могущество».** Руководство зовёт Potence «МОГУЩЕСТВОМ»,
основная книга — «Мощью». Приведено к основной книге, как и «КОЛДОВСТВО
КРОВИ» → «Кровавое чародейство».

**Коллизии с недостатками.** Rarefied Tastes переведено «Изысканным вкусом»,
а не «разборчивостью», потому что «разборчивость» уже занята недостатком
питания. По той же причине Repulsiveness — «Отвратительность», а не
«Омерзительность».

**Атрибуты и Навыки — с заглавной.** Издание пишет их строчными вперемешку
с прозой: «обаяние + Величие против самообладания + интеллекта». На книжной
полосе термин находит вёрстка, в карточке Foundry её нет. Регистр поднят
намеренно, и вместе с курсивом это единственное расхождение с изданием на
уровне букв. Поднимается только там, где слово однозначно термин: в формуле
пула, после опоры («проверка», «показатель», «против») или перед цифрой
рейтинга. «Сила подействует на одно животное» остаётся как есть — это сила
Дисциплины, а не Атрибут.

**Фанатский перевод помечен.** Бану Хаким, Геката, Ласомбра и Министерство
переведены только в Руководстве для игроков; в этих записях источник указан
прямо в тексте, чтобы отличать их от официального.
